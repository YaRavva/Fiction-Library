...
**Дата последнего обновления**: 2026-07-07 16:25

## Текущий Фокус

**Основные страницы получили единый градиентный фон, а «Мои книги» очищена от крупных счетчиков**: фон сделан статическим CSS-градиентом без рискованных Chromium-паттернов.

## Контекст Сессии

- Убраны крупные карточки-счетчики «Читаю сейчас» и «Избранное» из `src/app/my-books/page.tsx`; счетчики перенесены в компактные бейджи рядом с заголовками секций.
- Добавлена общая CSS-утилита `app-main-gradient` в `src/app/globals.css` для светлой и темной темы.
- Градиентный фон подключен к основным пользовательским, auth, admin и старым library/book-table оболочкам страниц.
- Фон реализован только через статические `linear-gradient`, без `backdrop-filter`, `mix-blend-mode`, hover `will-change` и JS-слушателей.
- Проверка: `bun x biome check --write ...`, `bun x tsc --noEmit --pretty false`, `bun run build`.
