...
import { BookOpen, Database, AlertCircle, CheckCircle, File } from 'lucide-react'
...
export function TelegramStatsSection() {
  ...
  const loadStats = async () => {
    try {
      ...
      setError(`Ошибка загрузки статистики Telegram: ${err.message || 'Неизвестная ошибка'}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDownloadMissing = async () => {
    try {
      ...
      setError(`Ошибка при загрузке отсутствующих книг: ${err.message || 'Неизвестная ошибка'}`)
      setLogs(prev => [...prev, `Ошибка при загрузке отсутствующих книг: ${err.message || 'Неизвестная ошибка'}`])
    } finally {
