...
export async function GET(request: NextRequest) {
  try {
    ...
    if (countError) {
      throw new Error(`Ошибка получения общего количества книг: ${countError.message}`);
    }
    ...
    if (processedError) {
      throw new Error(`Ошибка получения количества обработанных книг: ${processedError.message}`);
    }
    stats.processedBooks = processedBooks || 0;

    // Получаем количество необработанных книг
    const { count: unprocessedBooks, error: unprocessedError } = await supabaseAdmin
      .from('books')
      .select('*', { count: 'exact', head: true })
      .eq('metadata_processed', false);

    if (unprocessedError) {
      throw new Error(`Ошибка получения количества необработанных книг: ${unprocessedError.message}`);
    }
    ...
    if (recentError) {
      throw new Error(`Ошибка получения списка необработанных книг: ${recentError.message}`);
    }
    ...
    if (messagesError) {
      throw new Error(`Ошибка получения количества обработанных сообщений: ${messagesError.message}`);
    }
    stats.processedMessages = processedMessages || 0;

    // Вычисляем процент завершения
    stats.completionPercentage = totalBooks && processedBooks ? 
      Math.round((processedBooks / totalBooks) * 100) : 0;
