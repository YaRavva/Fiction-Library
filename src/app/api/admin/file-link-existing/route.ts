...
export async function POST(request: NextRequest) {
	try {
		...
		console.log(
			`🔗 Начинаем привязку существующего файла к книге ${bookInfo}...`,
		);
		...
		try {
			...
			if (fileExists) {
				...
				if (mimeType !== newMimeType) {
					console.log(
						`⚠️ Тип файла отличается (существующий: ${mimeType}, новый: ${newMimeType}), возвращаем ошибку для повторной загрузки...`,
					);
					return NextResponse.json(
					...
					console.log(
						`⚠️ Размер файла отличается (существующий: ${existingFileSize}, новый: ${newFileBuffer.length}), возвращаем ошибку для повторной загрузки...`,
					);
					return NextResponse.json(
						{ error: "FILE_MISMATCH_NEEDS_REUPLOAD" },
						{ status: 422 }, // Unprocessable Entity
			...
			return NextResponse.json(
				{
					error: "Failed to link existing file to book",
