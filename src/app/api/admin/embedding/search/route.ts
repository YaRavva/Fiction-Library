...
import { generateEmbedding, prepareBookText, cosineSimilarity } from "@/lib/embedding-service";
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { filename, title, author, model, limit = 10, threshold = 0.4 } = body;
        ...
        try {
            const { data: vectorResults, error: vectorError } = await serverSupabase
                .rpc("match_books", {
            ...
            console.warn("Vector search function not available, falling back to cosine similarity");
        }
