...
export async function POST(request: NextRequest) {
	try {
		...
		try {
			...
			const { fileName, mimeType } = await fileLinkService.getFileInfo(
				fileMessageId,
				channelId,
			);
			...
			try {
				...
				if (fileMetadata) {
					...
					fileExists = true;
					console.log(
						`✅ Файл ${fileName} найден в S3 с размером ${existingFileSize} байт`,
					);
				}
			} catch (error) {
				// Файл не найден или другая ошибка
				console.log(
					`ℹ️ Файл ${fileName} не найден в S3 (будет загружен новый):`,
			...
			if (fileExists) {
				...
				} = await fileLinkService.downloadAndUploadFile(
					fileMessageId,
					channelId,
					book,
				);
				...
				if (mimeType !== newMimeType) {
					console.log(
						`⚠️ Тип файла отличается (существующий: ${mimeType}, новый: ${newMimeType}), загружаем новый файл...`,
					);
					...
					await fileLinkService.uploadToStorage(
						newFileName,
						newFileBuffer,
						newMimeType,
					);
					...
					console.log(
						`✅ Тип и размер файла совпадают, привязываем существующий файл...`,
					);
					...
					if (result.success) {
						console.log(
							`✅ Существующий файл успешно привязан к книге ${bookInfo}`,
						);
						return NextResponse.json({
							success: true,
						...
						console.error(
							`❌ Ошибка привязки существующего файла:`,
							result.error,
						);
						return NextResponse.json(
					...
					console.log(
						`⚠️ Размер файла отличается (существующий: ${existingFileSize}, новый: ${newFileBuffer.length}), заменяем файл...`,
					);
					// Удаляем старый файл и загружаем новый
					const { deleteObject } = await import("@/lib/s3-service");
					await deleteObject(s3Key, bucketName);
				...
				console.log(
					`📥 Файл не найден в S3, загружаем новый файл ${fileName} из канала ${channelId}...`,
				);
				// Если файл не существует, загружаем новый
				const result = await fileLinkService.processFileForBook(
					fileMessageId,
					channelId,
					book,
				);
			...
			return NextResponse.json(
				{
					error: "Failed to link file to book",
					details: (linkError as Error).message,
				},
				{ status: 500 },
			);
		}
