	...
	try {
		...
		if (timerSettings.last_run) {
			...
			if (timeSinceLastRun < MIN_INTERVAL_BETWEEN_RUNS) {
				console.log(
					`⚠️  Skipping download due to anti-spam protection. Last run was ${Math.floor(timeSinceLastRun / 1000)} seconds ago`,
				);
				return NextResponse.json({
		...
		console.log(
			`📚 Starting file download process (max ${MAX_FILES_PER_RUN} files)`,
		);
		const results =
			await syncService.downloadAndProcessFilesDirectly(MAX_FILES_PER_RUN);

		console.log(
			`✅ Cron file download completed. Processed ${results.length} files`,
		);
		...
		if (syncService) {
			try {
				...
				console.warn(
					"⚠️  Error during Telegram client shutdown after main error:",
					shutdownError,
				);
			}
		}
