...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const { data: books, error: booksError } = await supabaseAdmin
			.from("books")
			.select(
				"id, title, author, publication_year, file_url, file_size, file_format, telegram_file_id, created_at, updated_at",
			)
		...
		for (const book of books) {
			try {
				...
				if (duplicateCheck.exists && duplicateCheck.book) {
					...
					if (!existingBook.file_url && book.file_url) {
						...
						if (updateError) {
							results.details.push(
								`Failed to update book ${existingBook.id} with file: ${updateError.message}`,
							);
						} else {
							results.details.push(
								`Updated book ${existingBook.id} with file from duplicate ${book.id}`,
							);
							...
							if (deleteError) {
								results.details.push(
									`Failed to delete duplicate book ${book.id}: ${deleteError.message}`,
								);
							} else {
					...
					// Если у обеих книг есть файлы, оставляем существующую и удаляем дубликат
						...
						if (deleteError) {
							...
							results.duplicatesRemoved++;
							results.details.push(
								`Deleted duplicate book ${book.id} (both had files)`,
							);
						}
					}
					// Если ни у одной книги нет файла, удаляем дубликат
						...
						if (deleteError) {
							...
							results.duplicatesRemoved++;
							results.details.push(
								`Deleted duplicate book ${book.id} (no files)`,
							);
						}
					}
				}
				...
				results.details.push(
					`Error processing book ${book.id}: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		}
