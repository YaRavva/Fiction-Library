...
/**
 * Скрипт для загрузки обложек книг из Telegram-канала в уже существующий бакет Cloud.ru S3
 */

async function uploadCoversToExistingBucket() {
  console.log('🚀 Начинаем загрузку обложек книг из Telegram-канала в Cloud.ru S3');
  ...
  try {
    ...
    for (let i = 0; i < booksWithoutCovers.length; i++) {
      ...
      try {
        ...
        // Загружаем обложку в Cloud.ru S3 (используем уже существующий бакет)
...
export { uploadCoversToExistingBucket };