 ...
 */
async function checkDuplicateBook(title: string, author: string): Promise<boolean> {
  ...
  const searchPromises = allSearchWords.map(async (word) => {
    ...
    const allMatches: BookRecord[] = [...(titleMatches || []), ...(authorMatches || [])];
  ...
  for (const match of topMatches) {
    ...
    if (matchTitle === normalizedTitle && matchAuthor === normalizedAuthor) {
      console.log(`⚠️  Exact duplicate found: "${match.title}" by ${match.author} (ID: ${match.id})`);
      return true;
    }
    ...
    if (relevance >= 0.75) {
      console.log(`⚠️  High relevance duplicate found: "${match.title}" by ${match.author} (ID: ${match.id}, relevance: ${(relevance * 100).toFixed(1)}%)`);
      return true;
    }
  }
...
async function addBookMetadata() {
  try {
    ...
    if (messages && messages.length > 0) {
      console.log(`📚 Found ${messages.length} messages. Processing each one with deduplication...`);
      ...
      for (const message of messages) {
        ...
        if (checkError && checkError.code !== 'PGRST116') { // PGRST116 means no rows returned
          console.error(`❌ Error checking if message ${messageId} is already processed:`, checkError);
          skippedCount++;
          continue;
        }
        ...
        try {
          ...
          const isDuplicate = await checkDuplicateBook(metadata.title, metadata.author);
          if (isDuplicate) {
            console.log(`⏭️ Message ${messageId} is a duplicate. Skipping.`);
            duplicateCount++;
            // Помечаем сообщение как обработанное, чтобы не проверять его снова
            const processedTable: any = serverSupabase.from('telegram_processed_messages');
          ...
          if (message.media) {
            ...
            try {
              ...
              if (buffer) {
                ...
                console.log(`✅ Cover uploaded to S3 successfully: ${coverUrl}`);
              }
            } catch (error) {
              console.error(`❌ Error uploading cover for message ${messageId} to S3:`, error);
            }
          }
          ...
          if (metadata.books && metadata.books.length > 0) {
            ...
            const seriesData: any = {
              ...
              series_composition: metadata.books.map(b => ({ title: b.title, year: b.year })),
            };
            if (coverUrl) {
            ...
            if (seriesResult.error) {
              console.error(`❌ Error inserting series record for message ${messageId}:`, seriesResult.error);
            } else {
              // Get the inserted series ID
              // @ts-ignore
              const { data: newSeries, error: selectError }: any = await serverSupabase
                .from('series')
                .select('id')
              ...
              if (selectError) {
                console.error(`❌ Error retrieving series record for message ${messageId}:`, selectError);
              } else {
                seriesId = newSeries.id;
                console.log(`✅ Series record created successfully with ID: ${seriesId}`);
              }
            }
          }
          ...
          if (bookResult.error) {
            console.error(`❌ Error inserting book metadata for message ${messageId}:`, bookResult.error);
          } else {
            console.log(`✅ Book metadata inserted successfully for message ${messageId}`);
            
            // Get the inserted book ID
            // @ts-ignore
            const { data: bookRecord, error: selectError }: any = await serverSupabase
              .from('books')
              .select('id')
              .eq('telegram_post_id', messageId.toString())
              .single();
            ...
            if (selectError) {
              console.error(`❌ Error retrieving book record for message ${messageId}:`, selectError);
            } else {
              // Mark this message as processed
              // Workaround for TypeScript typing issue
              const processedTable: any = serverSupabase.from('telegram_processed_messages');
              const { error: processedError }: any = await processedTable.insert([{
                message_id: messageId.toString(),
                channel: metadataChannel.id.toString(),
              ...
              if (processedError) {
                console.error(`❌ Error marking message ${messageId} as processed:`, processedError);
              } else {
