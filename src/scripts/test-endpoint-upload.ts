...
async function testEndpointUpload() {
  console.log('=== Тестирование загрузки файлов через endpoint https://s3.cloud.ru/books ===');
  ...
  try {
    ...
    const testFileName = `endpoint-test-${Date.now()}.txt`;
    const testContent = 'Тестовый файл для проверки загрузки через endpoint\nДата: ' + new Date().toISOString();
    const testContentBuffer = Buffer.from(testContent, 'utf-8');
    
    // Подготовка параметров для аутентификации
    // Используем путь к файлу относительно бакета
    const requestParams = {
      method: 'PUT',
      pathname: `/${testFileName}`, // Путь к файлу относительно бакета
      query: {},
      headers: {
        'host': 's3.cloud.ru',
