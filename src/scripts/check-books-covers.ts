...
async function checkBooksCovers() {
  ...
  try {
    ...
    const { count: booksWithCovers, error: coversCountError } = await supabaseAdmin
      .from('books')
      .select('*', { count: 'exact', head: true })
      .not('cover_url', 'is', null);

    if (coversCountError) {
      console.error('Ошибка при получении количества книг с обложками:', coversCountError);
      return;
    }
    ...
    // Показываем процент
    const coveragePercentage = totalBooks ? ((booksWithCovers || 0) / totalBooks * 100).toFixed(2) : 0;
    ...
    if (booksWithoutCovers > 0) {
      ...
      const { data: booksWithoutCoversData, error: sampleError } = await supabaseAdmin
        .from('books')
