...
async function testExactMessageId() {
  try {
    console.log('🔍 Тестируем получение сообщений с точным ID из базы данных');
    ...
    for (const book of books) {
      try {
        ...
        // Получаем сообщение с точным ID
    ...
    if (totalCount > 0 && successCount > 0) {
      console.log(`\n✅ Метод с точным ID работает в ${Math.round((successCount / totalCount) * 100)}% случаев`);
    } else {
      console.log(`\n❌ Метод с точным ID не работает`);
    }
