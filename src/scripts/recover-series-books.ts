            ...
            for (let i = 0; i < composition.length; i++) {
                ...
                if (existingBook) {
                    ...
                    if (existingBook.file_url) {
                        ...
                        console.log(`   🔸 [${seriesOrder}] "${bookTitle}" - существует, но БЕЗ ФАЙЛА`);
                ...
                console.log(`   ❌ [${seriesOrder}] "${bookTitle}" - ОТСУТСТВУЕТ в БД. Ищем в Telegram...`);

                // Ищем в Telegram
                // Ищем по названию книги + фамилия автора (для точности)
                const searchKeywords = [bookTitle, series.author.split(' ').pop() || series.author];
                const searchResults = await metadataService.searchMessagesByKeywords(searchKeywords, 5);

                // Фильтруем результаты
                const bestMatch = searchResults.find(r => r.similarity >= 8); // Высокий порог схожести

                if (bestMatch) {
                    console.log(`      🎯 Найдено совпадение в Telegram! ID: ${bestMatch.message_id}, Sim: ${bestMatch.similarity}`);
                    ...
                    try {
                        ...
                        if (newBook) {
                            ...
                            console.log(`      ✅ Книга восстановлена и привязана к серии! ID: ${newBook.id}`);
                        } else {
                            console.warn(`      ⚠️ Книга скачана, но не найдена в БД для привязки (Ошибка: ${findError?.message})`);
                        }
