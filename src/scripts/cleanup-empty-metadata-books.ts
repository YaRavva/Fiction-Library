...
async function cleanupEmptyMetadataBooks() {
	try {
		...
		console.log(
			`📚 Найдено книг с пустыми метаданными: ${emptyMetadataBooks?.length || 0}`,
		);
		...
		if (emptyMetadataBooks && emptyMetadataBooks.length > 0) {
			...
			emptyMetadataBooks.slice(0, 10).forEach((book: any, index: number) => {
				console.log(
					`${index + 1}. ID: ${book.id}, Автор: "${book.author}", Название: "${book.title}", Telegram ID: ${book.telegram_post_id || "Нет"}`,
				);
			});
			...
			console.log(
				`✅ Удалено ${emptyMetadataBooks.length} записей с пустыми метаданными.`,
			);
		} else {
