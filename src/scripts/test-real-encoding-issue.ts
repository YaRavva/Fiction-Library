...
async function testRealEncodingIssue() {
  console.log('🚀 Тестируем реальную проблему с кодировкой...\n');
  ...
  // Теперь проверим проблему с отображением в Telegram
  console.log(`\n--- Проблема с отображением в Telegram ---`);
  console.log('В Telegram имена файлов могут приходить в NFD форме,');
  console.log('в то время как в базе данных они хранятся в NFC форме.');
  console.log('Это может приводить к визуальным артефактам и проблемам с сопоставлением.');
  
  // Проверим, как выглядят строки в разных формах
  console.log(`\n🔍 Визуальное сравнение:`);
  console.log(`  NFC:  "${nfcString}"`);
  console.log(`  NFD:  "${nfdString}"`);
  console.log(`  Равны: ${nfcString === nfdString}`);
  
  // Проверим побайтово
  console.log(`\n🔢 Побайтовое сравнение:`);
  console.log(`  NFC байты:  [${Array.from(nfcString).map(char => char.charCodeAt(0)).join(', ')}]`);
  console.log(`  NFD байты:  [${Array.from(nfdString).map(char => char.charCodeAt(0)).join(', ')}]`);
  
  console.log('\n✅ Тестирование реальной проблемы с кодировкой завершено!');
}

// Запускаем тест
testRealEncodingIssue().catch(console.error);