...
async function testMessageIdMinusOne() {
  try {
    console.log('🔍 Тестируем получение сообщений с ID = сохраненный ID - 1');
    ...
    for (const book of books) {
      try {
        ...
        const targetMessageId = storedMessageId - 1; // Уменьшаем ID на 1
        ...
        // Получаем сообщение с уменьшенным ID
        ...
        if (!targetMessage) {
          ...
          // Проверим, какие сообщения были получены
          console.log(`  Полученные ID: ${messages.map(msg => msg.id).join(', ')}`);
          continue;
        }
    ...
    if (totalCount > 0 && successCount > 0) {
      console.log(`\n✅ Метод уменьшения ID на 1 работает в ${Math.round((successCount / totalCount) * 100)}% случаев`);
    } else {
      console.log(`\n❌ Метод уменьшения ID на 1 не работает`);
    }
