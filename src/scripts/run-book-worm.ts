    ...
    try {
        ...
        switch (mode) {
            ...
            case 'update':
                ...
                break;
            case 'index':
                console.log('🐋 Запуск Книжного Червя в режиме ИНДЕКСАЦИИ...');
                sendStatus('Начало индексации', 5);
                // Убираем вызов несуществующего метода advancedIndexMessages
                result = await bookWorm.runAutoSync(); // Используем существующий метод
                sendStatus('Индексация завершена', 100);
                break;
            case 'auto':
            default:
