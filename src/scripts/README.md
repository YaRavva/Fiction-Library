...
Тестовый скрипт для проверки полной синхронизации "Книжного Червя".

### Назначение

Этот скрипт позволяет протестировать полную синхронизацию "Книжного Червя", которая:
1. Проверяет наличие пропущенных сообщений в канале с метаданными
2. При необходимости выполняет полную синхронизацию всех сообщений
3. Сопоставляет файлы с книгами без файлов
...
Основной скрипт запуска "Книжного Червя" с выбором режима работы.

### Назначение

Этот скрипт запускает "Книжного Червя" в одном из трех режимов:
1. **Полная синхронизация** - обрабатывает все данные из каналов
2. **Обновление** - проверяет только новые сообщения
3. **Автоматический** - сам определяет, какой режим использовать
...
Скрипт для запуска полной синхронизации "Книжного Червя".

### Назначение

Этот скрипт запускает "Книжного Червя" в режиме полной синхронизации:
1. Обрабатывает все сообщения из канала метаданных
2. Привязывает все доступные файлы к книгам
...
Скрипт для запуска запланированной синхронизации "Книжного Червя".

### Назначение

Этот скрипт запускает "Книжного Червя" в режиме обновления по расписанию:
1. Проверяет новые сообщения в канале метаданных
2. Привязывает новые файлы к книгам

### Использование

```bash
# Запуск с интервалом (по умолчанию каждые 30 минут)
npx tsx src/scripts/scheduled-book-worm.ts

# Запуск с пользовательским интервалом (например, каждые 60 минут)
...
### run-book-worm.ts
Запуск "Книжного Червя" в различных режимах:
...
### full-sync-book-worm.ts
Запуск полной синхронизации "Книжного Червя" (ручной режим):
- Обрабатывает все сообщения из канала метаданных
- Привязывает все доступные файлы к книгам
...
### scheduled-book-worm.ts
Запуск запланированной синхронизации "Книжного Червя" (автоматический режим):
- Проверяет новые сообщения в канале метаданных
- Привязывает новые файлы к книгам
...
# Запуск с пользовательским интервалом
BOOK_WORM_INTERVAL=60 npx tsx src/scripts/scheduled-book-worm.ts
```
...
## Режимы работы "Книжного Червя"

### 1. Автоматический режим (run-book-worm.ts без параметров)
- Проверяет, нужна ли полная синхронизация
- Если да - запускает полную синхронизацию
- Если нет - запускает режим обновления

### 2. Полная синхронизация (run-book-worm.ts full или full-sync-book-worm.ts)
- Обрабатывает все сообщения из канала метаданных
- Привязывает все доступные файлы к книгам
- Используется для первоначальной синхронизации или при обнаружении пропущенных данных

### 3. Режим обновления (run-book-worm.ts update или scheduled-book-worm.ts)
- Проверяет только новые сообщения в канале метаданных
- Привязывает только новые файлы
- Оптимизирован для частого автоматического запуска
...
- Проверяют наличие пропущенных сообщений и выполняют полную синхронизацию при необходимости
