...
async function updateSeriesWithOffset() {
  try {
    console.log('🚀 Начинаем обновление состава серии с учетом смещения ID');
    ...
    // Получаем сообщение с учетом смещения
    console.log(`📥 Получаем сообщение с ID: ${targetMessageId} (с учетом смещения)...`);
    
    // Для получения конкретного сообщения по ID, используем offsetId равный targetMessageId
    ...
    if (!targetMessage) {
      console.log(`ℹ️  Точное сообщение с ID ${targetMessageId} не найдено в результатах`);
      return;
    }
