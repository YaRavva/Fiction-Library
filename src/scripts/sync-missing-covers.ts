...
export async function syncMissingCovers(limit?: number) {
  try {
    console.log(`🚀 Запуск синхронизации обложек для книг без обложек (лимит: ${limit ?? 'все'})`);
    ...
    if (fetchError) {
      throw new Error(`Ошибка получения книг без обложек: ${fetchError.message}`);
    }
    ...
    for (const book of booksWithoutCovers) {
      try {
        ...
        console.log(`  📥 Получаем сообщение ${typedBook.telegram_post_id} из Telegram...`);
    ...
    console.log(`✅ Синхронизация обложек завершена: ${processed} обработано, ${updated} обновлено, ${skipped} пропущено, ${errors} ошибок`);
    ...
    return {
      success: false,
      message: error instanceof Error ? error.message : 'Неизвестная ошибка синхронизации обложек',
      processed: 0,
      updated: 0,
      skipped: 0,
      errors: 1
