...
/**
 * Скрипт для загрузки обложек книг из Telegram-канала в Cloud.ru S3 без создания локальных резервных копий
 */

async function uploadCoversNoBackup() {
  console.log('🚀 Начинаем загрузку обложек книг из Telegram-канала в Cloud.ru S3 (без резервных копий)');
  ...
  try {
    ...
    for (let i = 0; i < booksWithoutCovers.length; i++) {
      ...
      try {
        ...
        // Загружаем обложку в Cloud.ru S3 (без создания локальных резервных копий)
