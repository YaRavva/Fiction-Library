...
async function testEndpointUploadHost() {
  console.log('=== Тестирование загрузки файлов через endpoint с правильным host ===');
  ...
  try {
    ...
    const testFileName = `endpoint-test-host-${Date.now()}.txt`;
    const testContent = 'Тестовый файл для проверки загрузки через endpoint с правильным host\nДата: ' + new Date().toISOString();
    const testContentBuffer = Buffer.from(testContent, 'utf-8');
    
    // Подготовка параметров для аутентификации
    // Попробуем указать правильный host для endpoint
    const requestParams = {
      method: 'PUT',
      pathname: `/${testFileName}`, // Путь к файлу относительно endpoint
      query: {},
      headers: {
        'host': 's3.cloud.ru', // Возможно, нужно указать host с бакетом
    ...
    if (response.ok) {
      ...
      console.log('Host в заголовках:', authHeaders['host']);
