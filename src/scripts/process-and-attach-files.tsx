...
async function processAndAttachFiles() {
    console.log('=== ОБРАБОТКА И ПРИВЯЗКА ФАЙЛОВ ИЗ TELEGRAM ===\n');
    ...
    try {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (exactMatch) {
                ...
                try {
                    await syncService.processFile(msg);
                ...
                if (potentialMatches.length > 0) {
                    ...
                    console.log(`     🎯 ВЫБРАНО ЛУЧШЕЕ СОВПАДЕНИЕ:`);
                    console.log(`        Название: "${bestMatch.title}"`);
                    console.log(`        Автор: ${bestMatch.author}`);
                    console.log(`        ID: ${bestMatch.id}`);
                    
                    // Спрашиваем подтверждение у пользователя
                    console.log(`     ❓ Хотите привязать файл к этой книге? (y/n)`);
                    // В автоматическом режиме выбираем "да" для демонстрации
                    console.log(`     ✅ Автоматический выбор: Да`);
                    
                    // Загружаем и привязываем файл
                    console.log(`     📤 Загрузка и привязка файла...`);
                    try {
                        await syncService.processFile(msg, bestMatch.id); // Передаем ID книги
                        console.log(`     ✅ Файл успешно загружен и привязан к книге`);
                    } catch (processError) {
                        console.log(`     ❌ Ошибка при загрузке/привязке файла: ${processError}`);
                    }
...
processAndAttachFiles().catch(console.error);