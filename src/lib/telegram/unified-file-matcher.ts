...
async function checkEmbeddingAvailable(
  ...
  try {
    ...
    return !error || !error.message.includes("function") || error.message.includes("match_books");
  } catch {
    return false;
  }
}
...
async function searchByEmbedding(
  ...
  limit: number = 20,
  messageId?: number,
): Promise<{ id: string; title: string; author: string; similarity: number }[]> {
  try {
...
export async function matchFileToBook(
  ...
  if (useEmbeddings) {
    ...
    if (embeddingAvailable) {
      ...
      const embeddingMatches = await searchByEmbedding(supabase, fileText, 20, file.message_id);
      ...
      if (embeddingMatches.length > 0) {
        ...
        const hybridResults: UnifiedMatchResult[] = topCandidates.map(
          (lexical) => {
            ...
            const hybridScore =
              Math.round(lexical.score * 0.6 + embeddingScore * 0.4);
