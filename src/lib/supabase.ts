...
export const supabase = new Proxy(
	{} as ReturnType<typeof createSupabaseClient>,
	{
		get(_target, prop) {
			return getSupabase()[
				prop as keyof ReturnType<typeof createSupabaseClient>
			];
		},
	},
);
...
export async function uploadFileToStorage(
	...
	if (!admin) {
		throw new Error(
			"SUPABASE_SERVICE_ROLE_KEY is not set. Cannot upload files to storage.",
		);
	}
...
export async function upsertBookRecord(book: Partial<Book>) {
	...
	if (!admin) {
		throw new Error(
			"SUPABASE_SERVICE_ROLE_KEY is not set. Cannot upsert book record.",
		);
	}

	console.log(
		`🔍 Ищем книгу по метаданным: title="${book.title}", author="${book.author}"`,
	);
	...
	if (book.title && book.author) {
		...
		if (!fetchError && existingBook) {
			console.log(
				`✅ Найдена существующая книга (точное совпадение): ${(existingBook as { id: string }).id}`,
			);
			...
			if (book.telegram_post_id)
				updateData.telegram_post_id = book.telegram_post_id; // Добавляем обработку telegram_post_id
		...
		console.log(
			`🔍 Пробуем поиск с релевантностью для: title="${book.title}", author="${book.author}"`,
		);
		...
		if (allSearchWords.length > 0) {
			...
			const matchesWithScores = uniqueMatches.map((bookItem) => {
				const typedBookItem = bookItem as {
					id: string;
					title: string;
					author: string;
				};
				const bookTitleWords = typedBookItem.title
					.normalize("NFC")
					.toLowerCase()
					.split(/\s+/);
				const bookAuthorWords = typedBookItem.author
					.normalize("NFC")
					.toLowerCase()
					.split(/\s+/);
				...
				for (const searchWord of allSearchWords) {
					...
					for (const bookWord of allBookWords) {
						...
						if (
							normalizedBookWord.includes(normalizedSearchWord) ||
							normalizedSearchWord.includes(normalizedBookWord)
						) {
							score++;
							_found = true;
			...
			if (relevantMatches.length > 0) {
				console.log(
					`✅ Найдено ${relevantMatches.length} релевантных совпадений:`,
				);
				relevantMatches.forEach((match, index) => {
					console.log(
						`  ${index + 1}. "${match.title}" автора ${match.author} (релевантность: ${match.score})`,
					);
				});
	...
	console.log(
		`➕ Создаем новую книгу: title="${book.title}", author="${book.author}"`,
	);
