	...
	try {
		...
		if (!accessKeyId || !secretAccessKey) {
			return {
				success: false,
				error:
					"Не установлены необходимые переменные окружения: AWS_ACCESS_KEY_ID или AWS_SECRET_ACCESS_KEY",
			};
		}
		...
		if (accessKeyParts.length !== 2) {
			return {
				success: false,
				error:
					'Неверный формат AWS_ACCESS_KEY_ID. Ожидается формат "tenantId:keyId"',
			};
		}
