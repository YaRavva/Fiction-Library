...
export async function uploadFile(
	bucketName: string,
	fileName: string,
	fileContent: Buffer | ReadableStream,
) {
	try {
...
export async function getFile(
	...
	try {
		...
		console.error(
			`Ошибка при получении файла ${fileName} из бакета ${bucketName}:`,
			error,
		);
		throw error;
	}
...
/**
 * Функция для генерации presigned URL для скачивания файла
 * @param bucketName - имя бакета
 * @param fileName - имя файла
 * @param expiresIn - время жизни ссылки в секундах (по умолчанию 1 час)
 * @returns Promise, который резолвится с presigned URL
 */
export async function getDownloadUrl(
	...
	try {
		...
		const url = await getSignedUrl(cloudRuS3, command, { expiresIn });
		return url;
	} catch (error) {
		console.error(
			`Ошибка при генерации presigned URL для ${fileName} в бакете ${bucketName}:`,
			error,
		);
		throw error;
	}
