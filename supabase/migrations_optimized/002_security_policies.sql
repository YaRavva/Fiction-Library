...
-- Optimized Migration 002: Row Level Security Setup
...
ALTER TABLE telegram_stats ENABLE ROW LEVEL SECURITY;
...
-- Telegram sync status policies (только админы)
CREATE POLICY "Only admins can manage telegram sync status" ON telegram_sync_status
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM user_profiles 
            WHERE id = auth.uid() AND role = 'admin'
...
-- Processed Telegram messages policies (только админы)
CREATE POLICY "Only admins can manage processed telegram messages" ON telegram_processed_messages
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM user_profiles 
            WHERE id = auth.uid() AND role = 'admin'
...
-- Telegram stats policies (только админы)
CREATE POLICY "Only admins can manage telegram stats" ON telegram_stats
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM user_profiles 
            WHERE id = auth.uid() AND role = 'admin'
