...
-- Optimized Migration 003: Utility Functions
...
-- Function to update Telegram sync status
CREATE OR REPLACE FUNCTION update_telegram_sync_status(
    channel_id BIGINT,
...
RETURNS void AS $$
BEGIN
    INSERT INTO telegram_sync_status (
        channel_id,
        last_message_id,
        last_sync_at,
        error_count,
    ...
    SET
        ...
        last_sync_at = NOW(),
        error_count = CASE 
            WHEN error_occurred THEN telegram_sync_status.error_count + 1
            ELSE telegram_sync_status.error_count
        END,
        is_active = TRUE,
        updated_at = NOW();
END;
...
-- Function to mark Telegram message as processed
CREATE OR REPLACE FUNCTION mark_telegram_message_processed(
    message_id BIGINT,
    channel_id BIGINT,
...
BEGIN
    ...
    ON CONFLICT (message_id, channel_id) DO UPDATE
    SET
...
-- Function to get Telegram sync statistics
CREATE OR REPLACE FUNCTION get_telegram_sync_stats()
RETURNS TABLE (
    total_messages_processed BIGINT,
    total_files_downloaded BIGINT,
    today_messages_processed BIGINT,
    today_files_downloaded BIGINT,
    error_count INTEGER
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        (SELECT COUNT(*) FROM telegram_processed_messages) as total_messages_processed,
        (SELECT COUNT(*) FROM telegram_processed_messages WHERE file_id IS NOT NULL) as total_files_downloaded,
        (SELECT COUNT(*) FROM telegram_processed_messages WHERE processed_at >= CURRENT_DATE) as today_messages_processed,
        (SELECT COUNT(*) FROM telegram_processed_messages WHERE file_id IS NOT NULL AND processed_at >= CURRENT_DATE) as today_files_downloaded,
        COALESCE((SELECT MAX(error_count) FROM telegram_sync_status), 0) as error_count;
END;
$$ LANGUAGE plpgsql;