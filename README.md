...
### Функциональность
- Автоматическая проверка дубликатов книг по автору и названию
- Объединение данных при частичном совпадении
- Загрузка отсутствующих файлов в хранилище
- Настройка таймеров через админ панель

### API Endpoints
- `POST /api/admin/deduplicate` - Запуск дедупликации вручную
- `GET /api/admin/deduplicate/status` - Получение статуса и настроек
- `PUT /api/admin/deduplicate/settings` - Обновление настроек таймера
- `GET /api/cron/deduplicate` - Автоматический запуск по расписанию

### Скрипты
- `src/scripts/automated-deduplication.ts` - Автономный скрипт для запуска дедупликации
- `src/scripts/enhanced-duplicate-check.ts` - Улучшенная проверка дубликатов

### Логика дедупликации
1. При совпадении по автору и названию:
   - Если у существующей книги нет файла, но у дубликата есть - дополнить данные
   - Если у обеих книг есть файлы - оставить существующую и удалить дубликат
   - Если ни у одной книги нет файла - удалить дубликат

2. При отсутствии файла у книги:
   - Загрузка файла в бакет с оригинальным именем
   - Связывание файла с книгой в базе данных
