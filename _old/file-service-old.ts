...
export class TelegramFileService {
	...
	public static async getInstance(): Promise<TelegramFileService> {
		if (!TelegramFileService.instance) {
			...
			TelegramFileService.instance.telegramClient =
	 ...
	 */
	public static extractMetadataFromFilename(filename: string): {
		author: string;
		title: string;
	} {
		...
		if (nameWithoutExt.includes("Хроники")) {
			...
			const chroniclesIndex = words.findIndex((word) =>
				word.includes("Хроники"),
			);

			if (chroniclesIndex > 0) {
				// Авторы - это слова до "Хроники"
				const authors = words
					.slice(0, chroniclesIndex)
					.join(" ")
					.replace(/_/g, " ")
					.trim();
				const title = words
					.slice(chroniclesIndex)
					.join(" ")
					.replace(/_/g, " ")
					.trim();
		...
		if (words.length >= 3) {
			...
			for (let i = 1; i < Math.min(words.length - 1, 4); i++) {
				// Проверяем до 3 слов для автора
				...
				if (
					titleKeywords.some((keyword) =>
						potentialTitle.normalize("NFC").toLowerCase().includes(keyword),
					)
				) {
					return {
	...
	private async downloadAndProcessSingleFile(message: {
		...
		try {
			...
			const { author, title } =
				TelegramFileService.extractMetadataFromFilename(normalizedFilename);
			...
			if (existingFile) {
				...
				// После скачивания проверяем размер файла и снова проверяем существование в бакете
				console.log(
					`  🔍 Повторная проверка существования файла в S3 бакете после скачивания: ${storageKey}`,
				);
				const existingFileAfterDownload = await this.checkExistingFileInS3(
					storageKey,
					buffer.length,
					mime,
				);
				...
				if (existingFileAfterDownload) {
					console.log(
						`  ✅ Файл уже существует в S3 бакете и совпадает по размеру и типу`,
					);
					// Формируем URL файла
					const bucketName = process.env.S3_BUCKET_NAME;
					if (!bucketName) {
					...
					// Файл не существует или не совпадает по размеру/типу, загружаем новый
		...
		const rankedMatches = matches
			.map((book) => {
				...
				if (
					...
					score += 100; // Увеличиваем вес точного совпадения названия
				}
				...
				if (
					...
					score += 50; // Увеличиваем вес точного совпадения автора
				}
				...
				if (
					...
					score += 100; // Дополнительный бонус за полное совпадение
				}
				...
				const _bookTitleWords = normalizedBookTitle
					.toLowerCase()
					.split(/\s+/)
					.filter((word) => word.length > 2);
				const searchTitleWords = normalizedTitle
					.toLowerCase()
					.split(/\s+/)
					.filter((word) => word.length > 2);
				...
				if (
					searchTitleWords.length > 0 &&
					titleWordsMatchCount / searchTitleWords.length >= 0.5
				) {
					score += 15;
				}
				...
				const allWords = normalizedTitle
					.toLowerCase()
					.split(/[_\-\s]+/)
				...
				let _titleWordsFound = 0;
				let _authorWordsFound = 0;
				...
				// Если все слова из имени файла присутствуют в названии книги
				if (allWordsInTitle) {
					score += 30; // Большой бонус за полное совпадение названия
				}
				// Если все слова из имени файла присутствуют в авторе книги
				if (allWordsInAuthor) {
					score += 30; // Большой бонус за полное совпадение автора
				}
				// Если все слова из имени файла присутствуют в названии и авторе книги
				if (allWordsInTitle && allWordsInAuthor) {
					score += 100; // Огромный бонус за полное совпадение названия и автора
				}
				// Если более 80% слов из имени файла присутствуют в названии и авторе книги
				if (wordsFoundCount / allWords.length >= 0.8) {
					score += 20; // Большой бонус за частичное совпадение
				}
	...
	/**
	 * Проверяет существование файла в S3 бакете и возвращает информацию о нем
	 * @param storageKey Ключ файла в S3
	 * @param expectedSize Ожидаемый размер файла
	 * @param expectedMime Ожидаемый MIME-тип файла
	 * @returns Данные о файле или null, если файл не существует или не соответствует ожидаемым параметрам
	 */
	private async checkExistingFileInS3(
		...
		try {
			...
			if (
				...
				response.ContentType === expectedMime
			) {
		...
		if (
			!normalizedTitle.toLowerCase().includes("цикл") &&
			normalizedBookTitle.toLowerCase().includes("цикл")
		) {
			// Убираем префикс "цикл" из названия книги и проверяем совпадение
			const bookTitleWithoutCycle = normalizedBookTitle
				.toLowerCase()
				.replace("цикл", "")
				.trim();
			if (
				bookTitleWithoutCycle.includes(normalizedTitle.toLowerCase()) ||
				...
				const titleWords = normalizedTitle
					...
					.split(/\s+/)
					.filter((word: string) => word.length > 2);
				const _bookTitleWordsWithoutCycle = bookTitleWithoutCycle
					.split(/\s+/)
					.filter((word: string) => word.length > 2);
				...
				if (
					titleWords.length > 0 &&
					cycleWordsMatchCount / titleWords.length >= 0.5
				) {
					score += 15;
				}
			}
		...
		const extractedTitleWords = normalizedTitle
			.toLowerCase()
			.split(/\s+/)
			.filter((word: string) => word.length > 2);
		const bookTitleWordsFiltered = normalizedBookTitle
			.toLowerCase()
			.split(/\s+/)
			.filter((word: string) => word.length > 2);
		...
		if (
			extractedTitleWords.length > 0 &&
			exactWordsMatchCount === extractedTitleWords.length
		) {
		...
		// Если совпадает большинство слов, добавляем средний бонус
			...
			extractedTitleWords.length > 0 &&
			exactWordsMatchCount / extractedTitleWords.length >= 0.7
		) {
			score += 25;
		}
		// Если совпадает более 50% слов, добавляем небольшой бонус
			...
			extractedTitleWords.length > 0 &&
			exactWordsMatchCount / extractedTitleWords.length >= 0.5
		) {
			score += 15;
		}
		...
		if (normalizedSearchTerms.length > 0) {
			const matchRatio =
				improvedWordMatchCount / (normalizedSearchTerms.length * 2); // Максимум 100% совпадения
