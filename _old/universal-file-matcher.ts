...
export class UniversalFileMatcher {
	...
	public static matchFileToBook(
		...
		const allBookAuthorWordsMatch =
			bookAuthorWords.length > 0 &&
			bookAuthorWords.every((bookAuthorWord) =>
				fileAuthorWords.includes(bookAuthorWord),
			);

		const allBookTitleWordsMatch =
			bookTitleWords.length > 0 &&
			bookTitleWords.every((bookTitleWord) =>
				fileTitleWords.includes(bookTitleWord),
			);
