...
import { type Book, uploadFileToStorage } from "../supabase";
import type { TelegramService } from "./client";
import type { TelegramFileService } from "./file-service";
import type { TelegramMetadataService } from "./metadata-service";
import type { BookMetadata } from "./parser";
...
export class SyncService {
	...
	constructor(
		metadataService: TelegramMetadataService,
		fileService: TelegramFileService,
	...
	public async fullMetadataSync(): Promise<{
		...
		try {
			...
			for (let i = 0; i < allMessages.length; i += batchSize) {
				...
				console.log(
					`  📦 Обработка пакета ${Math.floor(i / batchSize) + 1} из ${Math.ceil(allMessages.length / batchSize)}...`,
				);
				...
				console.log(
					`    → Обработано: ${result.processed}, Добавлено: ${result.added}, Обновлено: ${result.updated}, Пропущено: ${result.skipped}, Ошибок: ${result.errors}`,
				);
			...
			console.log(
				`  ✅ Полная синхронизация завершена. Всего обработано: ${totalProcessed}`,
			);
	...
	private async processMessageBatch(messages: unknown[]): Promise<{
		...
		try {
			...
			for (const msg of messages) {
				...
				try {
					...
					if (!error && data) {
						...
						console.log(
							`      ℹ️ Книга "${metadata.title}" автора ${metadata.author} найдена в БД.`,
						);
					}
				} catch (checkError) {
					console.warn(
				...
				if (existingBook) {
					if (
						existingBook.telegram_post_id === null ||
						existingBook.telegram_post_id !== (anyMsg.id as number)
					) {
						console.log(
							`      🔄 Обновляем telegram_post_id для книги "${metadata.title}" (${existingBook.id})`,
						);
						...
						if (updateError) {
							console.error(
								`      ❌ Ошибка при обновлении telegram_post_id:`,
								updateError,
							);
							errors++;
							details.push({
						...
						console.log(
							`      ✅ Книга "${metadata.title}" уже имеет корректный telegram_post_id, пропускаем`,
						);
						skipped++;
						details.push({
							msgId: anyMsg.id,
					...
					if (
						...
						try {
							...
							if (photoBuffer) {
								...
								console.log(
									`        → Загружаем в Storage: covers/${photoKey}`,
								);
								...
								console.warn(
									`        ⚠️ Не удалось скачать фото (пустой буфер)`,
								);
							}
						} catch (err: unknown) {
							console.error(
				...
				if (
					!existingBook ||
					(existingBook as Book).telegram_post_id === null ||
					(existingBook as Book).telegram_post_id !== (anyMsg.id as number)
				) {
					metadataList.push({
						...metadata,
						coverUrls:
			...
			console.log(
				`    📊 Всего подготовлено метаданных: ${metadataList.length}`,
			);
			...
			const _combinedDetails = [...details, ...resultImport.details];
	...
	private async getAllMessagesFromChannel(): Promise<unknown[]> {
		...
		try {
			...
			const channelId =
				typeof channel.id === "object" && channel !== null
	...
	public async syncFiles(): Promise<{
		...
		try {
			...
			const validBooks =
				booksWithoutFiles?.filter(
					(book) =>
						(book as { title: string }).title &&
						(book as { title: string }).title.trim() !== "" &&
						(book as { author: string }).author &&
						(book as { author: string }).author.trim() !== "",
				) || [];

			if (validBooks.length === 0) {
				console.log(
					"  ✅ Все книги имеют файлы или недостаточно данных для поиска",
				);
				return {
					processed: 0,
					linked: 0,
			...
			console.log(
				`  📚 Найдено ${validBooks.length} книг без файлов (с непустыми названиями и авторами)`,
			);
			...
			for (const book of validBooks) {
				...
				try {
					...
					if (matchingFile) {
						...
						const result = await this.fileService?.processSingleFileById(
						...
						if (result.success && !result.skipped) {
							...
							console.log(
								`    ⚠️  Файл пропущен: ${result.reason || "Неизвестная причина"}`,
							);
							skipped++;
						} else {
							console.log(
								`    ⚠️  Файл не привязан: ${result.reason || "Неизвестная причина"}`,
							);
							skipped++;
						}
					} else {
	...
	private findMatchingFile(book: Book, files: any[]): any | null {
		...
		for (const file of files) {
			...
			const authorInFilename = filename.includes(
				bookAuthor.replace(/\s+/g, "_"),
			);
			...
			if (
				bookTitleWords.length > 0 &&
				titleWordsMatchCount / bookTitleWords.length >= 0.5
			) {
				score += 15;
			}
			...
			const bookTitleContainsFalsePositive = falsePositiveKeywords.some(
				(keyword) => bookTitle.includes(keyword) && !filename.includes(keyword),
			);

			const filenameContainsFalsePositive = falsePositiveKeywords.some(
				(keyword) => filename.includes(keyword) && !bookTitle.includes(keyword),
			);
			...
			if (titleMatchThreshold > 0) {
				const partialTitle = bookTitle.substring(
					0,
					Math.min(titleMatchThreshold, bookTitle.length),
				);
			...
			if (authorMatchThreshold > 0) {
				const partialAuthor = bookAuthor.substring(
					0,
					Math.min(authorMatchThreshold, bookAuthor.length),
				);
			...
			const searchTerms = [
				...bookTitleWords,
				...bookAuthor.split(/\s+/).filter((word) => word.length > 2),
			];
		...
		if (bestMatch && bestScore >= 30) {
			console.log(
				`    ✅ Найдено совпадение с рейтингом ${bestScore}: ${bestMatch.filename}`,
			);
			return bestMatch;
		}
