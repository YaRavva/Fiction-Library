...
class MemoryBankUpdater {
  ...
  private readonly systemPatternsFile = join(this.memoryBankPath, 'systemPatterns.md')
  private readonly techContextFile = join(this.memoryBankPath, 'techContext.md')
  private readonly activeContextFile = join(this.memoryBankPath, 'activeContext.md')
  ...
  private getGitChanges(fromHash: string, toHash: string): GitChange[] {
    try {
      if (!fromHash) {
        ...
        const output = execSync('git diff --name-status HEAD~1 HEAD', { encoding: 'utf-8' })
        return this.parseGitOutput(output)
      }

      const output = execSync(`git diff --name-status ${fromHash} ${toHash}`, { encoding: 'utf-8' })
      return this.parseGitOutput(output)
    } catch (error) {
  ...
  private analyzeChanges(changes: GitChange[]): MemoryBankUpdate[] {
    ...
    for (const change of changes) {
      ...
      if (change.file.startsWith('src/components/') && change.file.endsWith('.tsx')) {
        updates.push({
      ...
      if (change.file.startsWith('src/lib/services/') && change.file.endsWith('.ts')) {
        updates.push({
          file: this.systemPatternsFile,
      ...
      if (change.file.startsWith('src/app/api/') && change.file.endsWith('.ts')) {
        updates.push({
          file: this.systemPatternsFile,
      ...
      if (change.file.startsWith('scripts/') || change.file.startsWith('src/scripts/')) {
        updates.push({
          file: this.techContextFile,
   ...
   */
  private updateProgressFile(newHash: string, updates: MemoryBankUpdate[]): void {
    try {
      ...
      if (updates.length > 0) {
        const updateInfo = updates.map(u => `- ${u.reason}: ${u.changes.join(', ')}`).join('\n')
        content = content.replace(
          /Status.*?✅[^\\n]*/,
  ...
  public async updateMemoryBank(checkOnly: boolean = false): Promise<void> {
    ...
    console.log(`📊 Анализ изменений: ${lastCheckedHash.substring(0, 8)} -> ${currentHash.substring(0, 8)}`)
    ...
    console.log(`🔄 Требуется обновление файлов Memory Bank: ${updates.length}`)
    ...
    console.log('💡 Рекомендуется просмотреть изменения и при необходимости дополнить документацию вручную')
  }
}
