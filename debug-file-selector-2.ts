...
async function debugFileSelector() {
  try {
    ...
    // Получаем файлы из таблицы telegram_stats (как в API)
    const { data: telegramStats, error: statsError } = await supabase
      .from('telegram_stats')
      .select('files')
      .limit(1);

    if (statsError) {
      throw new Error(`Ошибка получения статистики Telegram: ${statsError.message}`);
    }

    if (!telegramStats || telegramStats.length === 0 || !telegramStats[0].files) {
      console.log('❌ Нет данных о файлах в Telegram');
      return;
    }

    const allFiles = telegramStats[0].files;
    console.log(`📁 Всего файлов в Telegram: ${allFiles.length}`);
